...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   -->
    ...
    <xsl:template name="SL_no_series">
            <li><a href="#contents">Collection Contents</a></li>
        ...
        <div id="{@id}" class="c_row {name()} {name()}_row grid_12 alpha fa_odd">
            ...
            <div class='component_info'>
                <div class='component_title'>
                    <span><xsl:value-of select="did/unittitle"/><xsl:if test="did/unitdate">, <xsl:value-of select="did/unitdate"/></xsl:if></span>
                </div>
            </div>
    ...
    <xsl:template name="ns_containers">
        <div class="faid_containers">
            <span class="container_item box"><xsl:value-of select="did/unitid"/></span> 
        </div>  
    </xsl:template>
    ...
    <xsl:template name="series">
        ...
        <h2 class="mainheading">
            ...
            <xsl:if test="following-sibling::unitdate">, <xsl:value-of select="did/unitdate"/></xsl:if>
        </h2>
                            ...
                            <xsl:when test="@level='subseries' or @level='subgrp' or @level='series'">
                                <div class="section sectionSubSeries">
                                    <div class="title {name()} {name()}_title_row">
                                        <h3><span><xsl:value-of select="did/unittitle"/><xsl:if test="did/unitdate">, <xsl:value-of select="did/unitdate"/></xsl:if></span></h3>
                                        <xsl:if test="scopecontent">
                                            <p dir="auto"><b><xsl:value-of select="scopecontent/head"/><xsl:text>: </xsl:text></b><xsl:apply-templates select="scopecontent/p"/></p>
                                        </xsl:if>
                                        <xsl:if test="arrangement">
  ...
  <xsl:template name="even">
    ...
    <xsl:template name="cr_containers">
        <div class="faid_containers">
            <xsl:variable name="number" select="count(did/container/contains(@label,$cno))"/>
            <b><xsl:value-of select="$number"/> Box(es)</b>
            <xsl:choose>
                <xsl:when test="$number = 1">
                    <span class="container_item box fa_odd">Box <xsl:value-of select="did/container/@label"/></span>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when test="$number = 1"></xsl:when>
                        <xsl:when test="did/container/contains(@label,$cno)">
                            <span class="container_item box fa_odd">Boxes
                                <xsl:value-of select="did/container[1]/@label"/> - <xsl:value-of select="did/container[position() = last()]/@label"/>
                            </span>
                        </xsl:when>
                        <xsl:otherwise/>  
                    </xsl:choose>
